...
std::string npc_winzig::GetItemLink(uint32 entry, WorldSession* session)
{
...
std::string npc_winzig::GetItemIcon(uint32 entry, uint32 width, uint32 height, int x, int y)
{
...
void npc_winzig::UpdateItemCount(std::map<uint32, uint32> &entryToAmountMap, std::map<uint32, uint32> &entryToSubclassMap, Item* pItem, Player* player, uint32 bagSlot, uint32 itemSlot)
{
...
{
    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(37711, 30, 30, -18, 0) + "Vendor", VENDOR, 0);
    ...
    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(4500, 30, 30, -18, 0) + "Deposit All Reagents", DEPOSIT_ALL_REAGENTS, 0);
