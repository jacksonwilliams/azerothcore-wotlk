    ...
    public:
        ...
        void OnLogin(Player* player) override
        {
            ...
            if (localtime(&t)->tm_wday == Day::FRIDAY || localtime(&t)->tm_wday == Day::SATURDAY || localtime(&t)->tm_wday == Day::SUNDAY)
                ChatHandler(player->GetSession()).SendSysMessage("Weekend bonuses are now active!");
        }
};
...
class WeekendBonusWorld : WorldScript
{
    public:
        ...
        void OnAfterConfigLoad(bool reload) override
        {
            ...
            multiplierProfessions        = sConfigMgr->GetOption<uint32>("WeekendBonus.Multiplier.Professions", 1);
            multiplierReputation         = sConfigMgr->GetOption<float>("WeekendBonus.Multiplier.Reputation", 2.0f);
            multiplierProficiencies      = sConfigMgr->GetOption<uint32>("WeekendBonus.Multiplier.Proficiencies", 1);
        ...
        void OnUpdate(uint32 /*diff*/) override
        {
            ...
            {
                sWorld->SendServerMessage(SERVER_MSG_STRING, "Weekend bonuses are now active!");
                SetWorldRates(true);
                triggered = true;
            }
            else if (localtime(&localTime)->tm_wday == Day::MONDAY && triggered)
            {
                sWorld->SendServerMessage(SERVER_MSG_STRING, "Weekend bonuses are no longer active.");
                SetWorldRates(false);
                triggered = false;
            }
        }
